...
## Full-face hero task (Aug 15, 2026 21:30)
User clarifies: wants the FULL FACE shown in the hero, not a tight close-up crop. Current hero-portrait-face.png crop shows only partial face (eyes/nose/mouth). Plan: generate a NEW image via generate_image (3:4 or 2:3 portrait, full face visible head+shoulders, man with tortoiseshell round glasses, warm golden-hour skin tones matching warm ivory oklch(0.90 0.015 85) page bg, cinematic editorial, left side negative space warm beige for dissolve) saved to /home/ubuntu/webdev-static-assets/hero-full-face.png, then use the returned asset URL directly in Hero.tsx (PORTRAIT_IMG const). Keep Hero.tsx structure from checkpoint bada3700 (useHeroReveal staggered fade-in hook, portrait band w-[85%] md:w-[92%] with dissolve gradient to right). After image done: screenshot mobile+desktop, checkpoint (auto-publish), push +HEAD:main with GH_PUSH_TOKEN. Current checkpoint: bada3700.

## Task state (Aug 15, ~21:35) — image zoom + dimensions fix
User request: (1) make the new optical-vue image (Lunettes de vue card in Collections.tsx) match the dimensions and border radius of the sunglasses card exactly; (2) add subtle hover zoom to ALL images site-wide.
Current checkpoint: ddb14677 (pushed to GitHub main). Optical image URL: /manus-storage/optical-vue_69b33076.png. Local file /home/ubuntu/webdev-static-assets/optical-vue.png (5MB PNG).
Findings: Both collection cards already share identical markup (aspect-[4/3], object-cover, gold-frame, same group-hover:scale-[1.04] transition) — dimensions already match. Marques cards use aspect-[3/4] with the same hover zoom. About.tsx img uses aspect-[3/4] with NO hover zoom; hero portrait has none.
Next: verify optical image renders with matching aspect/radius in screenshot; add group-hover:scale-[1.04] transition to About.tsx img (and any other <img> lacking it); TS+build check, screenshot, checkpoint, push +HEAD:main with GH_PUSH_TOKEN, deliver.
